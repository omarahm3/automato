...
const (
	...
	ffmpeg_command = `ffmpeg -i %s -lavfi %s -vb %s -c:v libx264 -crf 20 ./blurry/%s.mp4 -n`
...
func ProcessAll(videos []downloader.Video) []ProcessedVideo {
	var (
		wg  sync.WaitGroup
		all []ProcessedVideo
	)
...
func process(video downloader.Video) (string, error) {
	cmdString := fmt.Sprintf(ffmpeg_command, video.Path, ffmpeg_filters, ffmpeg_quality, video.Post.Hash)
